...
{
    header('Location: Https://www.projectwolverine.net/mainpage.html');//you're still here so lets go to contact manager
    exit;
}
else if ( isset($_COOKIE['rememberme'] ))
{
    ...
    if( $count > 0 )
    {
        ...
        header('Location: Https://www.projectwolverine.net/mainpage.html');//I remember you, where we goin?
        exit;
    }
}
    ...
    if ($username != "" && $password != "")
    {
        ...
        if (mysqli_num_rows($result) > 0)
        {
            ...
            if (password_verify($password, $row ['Password']))
            {
                ...
                $_SESSION['userid'] = $userid;
                header('Location: http://www.ProjectWolverine.net/mainpage.html');//where we goin after loggin in?
                exit;
            }
        }
        else
