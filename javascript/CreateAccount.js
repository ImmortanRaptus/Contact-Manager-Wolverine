...
// 	//	alert('You must submit a valid email address');if (
// 	}
...
document.getElementById('eventButton').addEventListener('click', getValid)
;
    ...
    xhr.onload = function(){
        if(this.status == 200){
            ...
            else if (lists == 'Error: yes')
            {
            	document.getElementById("errorMessage").innerHTML= "Error: Duplicate username or email!"
            ...
            window.location.replace("https://www.projectwolverine.net/login.html");
            };
       
    	
       	const inputform= `User=${u.value}&Pass=${p.value}&Email=${e.value}`;
        console.log(u);
